...
// import highlighter icon 
//import { IconHighlighted, IconNotHighlighted } from 'icon-library';
import IconHighlighted from './IconHighlighted.png';
import IconNotHighlighted from './IconNotHighlighted.png';
...
export default function PracticeQuestion() {
  ...
  // define a state variable highlightActive using the useState hook 
  const [highlightActive, setHighlightActive] = useState<boolean>(false);
  
  // initialize highlightActive 
  ...
  return (
    <Paper p="xl" radius="md" withBorder>
      <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '1rem' }}>
        <Button
          variant={highlightActive ? 'filled' : 'light'}
          color={highlightActive ? 'dark' : 'gray'}
          onClick={toggleHighlight}
        >
          {highlightActive ? (
            <Image src={IconHighlighted} alt="IconHighlighted" width={100} height={100} />
          ) : (
            <Image src={IconNotHighlighted} alt="IconNotHighlighted" width={100} height={100} />
          )}
          <span>{highlightActive ? 'Deactivate Highlighter' : 'Activate Highlighter'}</span>
        </Button>
      </div>
        <form
          onSubmit={(e) => {
